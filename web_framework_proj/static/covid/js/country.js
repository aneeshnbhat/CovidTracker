...
var confirmed=[];
var xaxis=[];
for(i=country_data.length-30 ; i<country_data.length ; i++){
...
for(i=0 ; i<country_data.length ; i++){
    sum_confirmed = Math.max(country_data[i]['confirmed']);
    sum_deaths = Math.max(country_data[i]['deaths']);
    sum_recovered = Math.max(country_data[i]['recovered']);
}
